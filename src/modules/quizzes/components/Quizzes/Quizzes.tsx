...
import { useCreateQuizMutation, useGetGroupsQuery, useGetQuizzesQuery, useQuizCompletedQuery, useQuizIncommingStudentQuery } from '../../../../redux/api/apiSlice';
...
export default function Quizzes() {
  ...
  const {data: dataQuizzesIncomming, isLoading: isLoadingQuizzesIncomming} = useQuizIncommingStudentQuery()
  ...
  if (isLoadingQuizzesIncomming) return <Typography>Loading Quiz Completed ...</Typography>
  ...
  return (
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid size={{lg: 4, md: 4, sm: 12, xs: 12}}>
                        <Box>
                            ...
                            {userData?.role !== 'Instructor' ? '' : <select {...register('group', {
    ...
    {/* ========================== Code =========================== */}
      ...
      >
        ...
        <Grid container spacing={2}>
        {userData?.role !== 'Instructor' ? <Grid size={{lg: 6, md: 6, sm: 6, xs: 12}}>
                <Box className="grid-one-container">
                    <Link to={'/join-quiz'} className='grid-one-link'>
                        <Box className="grid-one">
                            <AddAlarmIcon sx={{fontSize: '60px'}}/>
                            <Typography className='grid-one-title'>Join Quiz</Typography>
                        </Box>
                    </Link>
                </Box>
            </Grid> : 
            ...
            <Grid size={{lg: 6, md: 6, sm: 6, xs: 12}}>
                <Box className="quizzes" sx={{border: '1px solid #00000033', borderRadius: '10px', padding: '15px'}}>
                    ...
                    {userData?.role !== 'Instructor' ? dataQuizzesIncomming.map((quiz: quizData) => {
